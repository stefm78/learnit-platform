...
def validate_role_b(manifest:Any,context:dict[str,Any],source_specs:list[str])->dict[str,Any]:
...
def run_gate(kit_path:Path,brief_path:Path,review_path:Path,source_specs:list[str],role_a_admission_paths:list[Path],role_b_manifest_path:Path)->dict[str,Any]:
    ...
    quality=_v5_quality(kit); policy=authoring_policy.analyze(kit); role_a=validate_role_a(kit,role_a_admission_paths); role_b=validate_role_b(_load_json(role_b_manifest_path,"Role B manifest"),context,source_specs); review,semantic_reasons=_validate_v5_review(_load_json(review_path,"V5 semantic review"),context); bindings=_binding_reasons(review,context)
