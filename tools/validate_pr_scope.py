...
def changed_files(base: str, head: str) -> list[dict[str, str]]:
    ...
    while index < len(tokens) and tokens[index]:
        ...
        if status.startswith(("R", "C")):
            old_path = normalize_path(tokens[index]); new_path = normalize_path(tokens[index + 1]); index += 2
            rows.append({"status": status, "path": new_path, "oldPath": old_path})
        else:
            path = normalize_path(tokens[index]); index += 1
...
def validate_scope(body: str, base: str, head: str) -> dict[str, Any]:
    ...
    for row in rows:
        ...
        for path in candidates:
            ...
            if path.lower().endswith(FORBIDDEN_ARCHIVE_SUFFIXES):
                violations.append(f"{path}: archive or executable payload is forbidden")
            if matches(path, forbidden):
                violations.append(f"{path}: explicitly forbidden by {wp_id}")
            elif not matches(path, allowed):
                violations.append(f"{path}: outside allowedPaths of {wp_id}")
    ...
    return {
        "schema": "learnit.pr_scope_report.v1",
        "ok": True,
        "workPackage": wp_id,
