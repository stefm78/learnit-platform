...
HEADER_RE = re.compile('(?m)^([a-z0-9_]+): ([^\\n]+)$')
...
def _publication_headers(body: str) -> dict[str, str] | None:
    marker_lines = re.findall(f'(?m)^{re.escape(OUTCOME_MARKER)}$', body)
...
def _matches_identity(body: str, *, repository: str, origin_type: str, origin_number: int, job_id: str, request_digest: str, target_sha: str) -> bool:
    return body.startswith(OUTCOME_MARKER + '\n') and f'job_id: {job_id}\n' in body and (f'request_sha256: {request_digest}\n' in body) and (f'repository: {repository}\n' in body) and (f'origin: {origin_type}#{origin_number}\n' in body) and (f'target_sha: {target_sha}\n' in body) and ('completion_state: FINAL_SEALED\n' in body or 'completion_state: FINAL_DIAGNOSTIC_ONLY\n' in body)
...
def _recompute_seal(file_digests: Any) -> tuple[str, str] | None:
    ...
    for name, digest in file_digests.items():
        if not isinstance(name, str) or not name or '\n' in name or ('\r' in name) or (not isinstance(digest, str)) or (not SHA256_RE.fullmatch(digest)):
            return None
...
def _verify_embedded_bundle_files(payload: dict[str, Any], file_digests: dict[str, str]) -> bool:
    """Recompute every bundle file whose bytes are present in the durable capsule."""
    facts = payload.get('facts')
    summary = payload.get('summary')
    if not isinstance(facts, dict) or not isinstance(summary, str):
        return False
    try:
        facts_text = json.dumps(facts, indent=2, sort_keys=True, ensure_ascii=False, allow_nan=False) + '\n'
...
def _cryptographically_complete_publication(body: str, *, repository: str, origin_type: str, origin_number: int, job_id: str, request_digest: str, target_sha: str) -> bool:
    """Reconstruct and verify durable capsule digests before restart trust."""
    ...
    if headers is None or payload is None or STATEMENT not in body:
        return False
    expected_headers = {'job_id': job_id, 'request_sha256': request_digest, 'repository': repository, 'origin': f'{origin_type}#{origin_number}', 'target_sha': target_sha}
    ...
    if not isinstance(origin, dict) or not isinstance(target, dict):
        return False
    if facts.get('job_id') != job_id or facts.get('request_sha256') != request_digest or facts.get('repository') != repository or (origin.get('type') != origin_type) or (origin.get('number') != origin_number) or (target.get('requested_sha') != target_sha) or (sealed.get('manifest_sha256') != manifest) or (sealed.get('bundle_sha256') != bundle):
        return False
    ...
    if recomputed is None or recomputed != (manifest, bundle):
        return False
    if not _verify_embedded_bundle_files(payload, file_digests):
        return False
    return True
...
class GhClient:
    ...
    def preflight(self, repository: str) -> dict[str, Any]:
        ...
        auth_record = self.runner.run(['gh', 'auth', 'status', '--active'], cwd=self.repository_root, timeout_seconds=30)
        ...
        repo_text = self._run(['gh', 'repo', 'view', repository, '--json', 'nameWithOwner,isPrivate,url,defaultBranchRef'], timeout=60)
        ...
        return {'gh_version': version[0] if version else '', 'authenticated_host': 'github.com', 'authenticated_login': user.get('login') if isinstance(user, dict) else None, 'repository': repo, 'credential_capabilities': 'not inferred from token scope', 'bridge_exposed_mutations': ['same-origin issue conversation comment creation']}
    ...
    def resolve_target(self, *, repository: str, target_type: str, target_number: int | None, target_sha: str) -> dict[str, Any]:
        if target_type == 'pull_request':
            ...
            return {'type': 'pull_request', 'number': target_number, 'sha': pr['head'].get('sha'), 'state': pr.get('state'), 'draft': pr.get('draft'), 'merged': pr.get('merged'), 'base_sha': pr.get('base', {}).get('sha') if isinstance(pr.get('base'), dict) else None, 'head_ref': pr.get('head', {}).get('ref') if isinstance(pr.get('head'), dict) else None}
    ...
    def collect_pr_snapshot(self, *, repository: str, pr_number: int, target_sha: str, include_logs: bool, include_artifacts: bool) -> tuple[dict[str, Any], dict[str, str], list[str]]:
        ...
        changed_files = _flatten_slurped_pages(self.api_json(f'repos/{repository}/pulls/{pr_number}/files?per_page=100', paginate=True, timeout=300))
        diff = self.api_text(f'repos/{repository}/pulls/{pr_number}', accept='application/vnd.github.v3.diff', timeout=300)
        artifacts['diff.patch'] = diff
        reviews = _flatten_slurped_pages(self.api_json(f'repos/{repository}/pulls/{pr_number}/reviews?per_page=100', paginate=True, timeout=300))
        artifacts['reviews.json'] = json.dumps(reviews, indent=2, sort_keys=True, ensure_ascii=False) + '\n'
        status_pages = self.api_json(f'repos/{repository}/commits/{target_sha}/status?per_page=100', paginate=True, timeout=300)
        ...
        check_runs = _flatten_slurped_pages(self.api_json(f'repos/{repository}/commits/{target_sha}/check-runs?per_page=100', paginate=True, timeout=300), list_key='check_runs')
        workflow_runs = _flatten_slurped_pages(self.api_json(f'repos/{repository}/actions/runs?head_sha={target_sha}&per_page=100', paginate=True, timeout=300), list_key='workflow_runs')
        ...
        for run in workflow_runs:
            if not isinstance(run, dict) or not isinstance(run.get('id'), int):
                ...
                continue
            run_id = run['id']
            jobs = _flatten_slurped_pages(self.api_json(f'repos/{repository}/actions/runs/{run_id}/jobs?per_page=100', paginate=True, timeout=300), list_key='jobs')
            workflow_jobs.extend(jobs)
            if include_logs:
                log_record = self.runner.run(['gh', 'run', 'view', str(run_id), '--repo', repository, '--log'], cwd=self.repository_root, timeout_seconds=600)
                if log_record.return_code == 0 and (not log_record.timed_out):
                    artifacts[f'workflow-run-{run_id}.log'] = log_record.stdout
                    log_summaries.append({'run_id': run_id, 'bytes': log_record.stdout_bytes, 'sha256': log_record.stdout_sha256, 'command_id': log_record.id})
                else:
                    missing_proof.append(f'WORKFLOW_LOG_UNAVAILABLE:{run_id}')
            if include_artifacts:
                workflow_artifacts.extend(_flatten_slurped_pages(self.api_json(f'repos/{repository}/actions/runs/{run_id}/artifacts?per_page=100', paginate=True, timeout=300), list_key='artifacts'))
        ...
        checks = {'combined_state': combined_state, 'status_contexts': status_contexts, 'check_runs': check_runs, 'workflow_runs': workflow_runs, 'workflow_jobs': workflow_jobs, 'workflow_artifacts': workflow_artifacts, 'log_summaries': log_summaries}
        artifacts['checks.json'] = json.dumps(checks, indent=2, sort_keys=True, ensure_ascii=False) + '\n'
        file_inventory = [{'filename': item.get('filename'), 'status': item.get('status'), 'additions': item.get('additions'), 'deletions': item.get('deletions'), 'changes': item.get('changes'), 'blob_url': item.get('blob_url'), 'raw_url': item.get('raw_url')} for item in changed_files if isinstance(item, dict)]
        metadata = {'number': pr_number, 'state': pr.get('state'), 'draft': pr.get('draft'), 'merged': pr.get('merged'), 'mergeable': pr.get('mergeable'), 'mergeable_state': pr.get('mergeable_state'), 'title': pr.get('title'), 'html_url': pr.get('html_url'), 'base': {'ref': pr.get('base', {}).get('ref') if isinstance(pr.get('base'), dict) else None, 'sha': pr.get('base', {}).get('sha') if isinstance(pr.get('base'), dict) else None}, 'head': {'ref': pr.get('head', {}).get('ref') if isinstance(pr.get('head'), dict) else None, 'sha': pr.get('head', {}).get('sha') if isinstance(pr.get('head'), dict) else None}, 'changed_files_count': len(file_inventory), 'changed_files': file_inventory, 'diff': {'bytes': len(diff.encode('utf-8')), 'sha256': hashlib.sha256(diff.encode('utf-8')).hexdigest(), 'artifact': 'diff.patch'}, 'reviews': reviews, 'checks': checks}
    ...
    def list_origin_comments(self, repository: str, origin_number: int) -> list[dict[str, Any]]:
        pages = self.api_json(f'repos/{repository}/issues/{origin_number}/comments?per_page=100', paginate=True, timeout=300)
        return [item for item in _flatten_slurped_pages(pages) if isinstance(item, dict)]

    def find_existing_final_publication(self, *, repository: str, origin_type: str, origin_number: int, job_id: str, request_digest: str, target_sha: str) -> dict[str, Any] | None:
        candidates: list[dict[str, Any]] = []
        for comment in self.list_origin_comments(repository, origin_number):
            body = comment.get('body')
            if isinstance(body, str) and _matches_identity(body, repository=repository, origin_type=origin_type, origin_number=origin_number, job_id=job_id, request_digest=request_digest, target_sha=target_sha):
        ...
        if not isinstance(body, str) or not isinstance(comment_id, int):
            return None
        if not _cryptographically_complete_publication(body, repository=repository, origin_type=origin_type, origin_number=origin_number, job_id=job_id, request_digest=request_digest, target_sha=target_sha):
            return None
    ...
    def publish_comment(self, *, repository: str, origin_number: int, body: str) -> PublicationResult:
        ...
        try:
            output = self._run(['gh', 'api', '-H', 'X-GitHub-Api-Version: 2022-11-28', '--method', 'POST', f'repos/{repository}/issues/{origin_number}/comments', '--input', str(payload_path)], timeout=300)
        finally:
        ...
        return PublicationResult(comment_id=value['id'], html_url=str(value.get('html_url', '')), body=str(value.get('body', '')), created_at=value.get('created_at') if isinstance(value.get('created_at'), str) else None)
    ...
    def verify_publication(self, *, repository: str, origin_number: int, result: PublicationResult, expected_body: str, required_fragments: Iterable[str]) -> dict[str, Any]:
        ...
        return {'state': 'VERIFIED', 'comment_id': result.comment_id, 'html_url': reread.get('html_url'), 'created_at': reread.get('created_at'), 'updated_at': reread.get('updated_at'), 'body_bytes': len(expected_body.encode('utf-8'))}
