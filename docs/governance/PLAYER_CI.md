...
The browser matrix is generated from `apps/player/dev/checks_registry.json`. The guard compares the registry with every `apps/player/tests/browser_*.py` file. A missing, stale or duplicate suite fails the inventory job before browser execution.
...
- `apps/player/dist/learnit.html`;
- `apps/player/reports/ci_artifact.json`;
- `apps/player/reports/aggregate_report.json`.
