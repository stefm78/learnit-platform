...
GOVERNANCE={"work-packages/PROG-WP-001.json"}
INT={"apps/learnit-next/build.py","apps/learnit-next/source_manifest.json","apps/learnit-next/dev/run_checks.py",".github/workflows/learnit-next-ci.yml"}; EXPECTED=set().union(*LANES.values(),BASELINE_QA_SUPPORT,GOVERNANCE,INT)
...
def provenance():
 ...
 if lane_paths&(INT|BASELINE_QA_SUPPORT|GOVERNANCE): raise GateError("ownership overlap")
 if (INT&BASELINE_QA_SUPPORT) or (INT&GOVERNANCE) or (BASELINE_QA_SUPPORT&GOVERNANCE): raise GateError("ownership overlap")
