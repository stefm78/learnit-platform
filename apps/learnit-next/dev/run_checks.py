...
BASELINE_TESTS = 30
EXPECTED_FULL_TESTS = 49
EXPECTED_BROWSER_TESTS = 6
...
EXPECTED_REVIEWS = {
    "runtime": 5087421021,
    "authoring": 4704571690,
    "qa": 5087471471,
}

P1_TEST = "apps/learnit-next/tests/p1_corrective_review.py"
HARNESS_TEST = "apps/learnit-next/tests/build_determinism.py"
...
def canonical_self_digest(manifest: dict) -> str:
    ...
    hits = [
        item
        for item in clone["workingFiles"]
        if item["path"] == SELF
    ]
    if len(hits) != 1:
        fail(
            "manifest self path is not unique",
            "provenance",
            "PROVENANCE_FAILURE",
        )
...
def validate_manifest(manifest: dict):
    ...
    if inputs != EXPECTED_INPUTS:
        fail(
            f"accepted inputs differ: {inputs}",
            "provenance",
            "PROVENANCE_FAILURE",
        )
    if manifest.get("acceptedReviews") != EXPECTED_REVIEWS:
        fail(
            "accepted reviews differ",
            "provenance",
            "PROVENANCE_FAILURE",
        )
    ...
    if (
        manifest.get("fileBudget") != 33
        or len(items) != 33
        or len(set(paths)) != 33
        or set(paths) != inventory()
    ):
        fail(
            "exact 33-file inventory differs",
            "provenance",
            "PROVENANCE_FAILURE",
        )
    ...
    if (
        ...
        or self_item["fingerprint"]["value"]
        != canonical_self_digest(manifest)
    ):
        fail(
            "manifest self fingerprint is stale",
            "provenance",
            "PROVENANCE_FAILURE",
        )
...
def integration_topology(base_ref: str):
    ...
    if not ancestor(base, "HEAD"):
        fail(
            f"base is not an ancestor of HEAD: {base}",
            "topology",
            "TOPOLOGY_FAILURE",
        )
    if git("merge-base", base_ref, "HEAD") != base:
        fail(
            f"merge-base differs from resolved base: {base}",
            "topology",
            "TOPOLOGY_FAILURE",
        )
    ...
    if (
        len(changed) != 9
        or set(changed) != INTEGRATION_ALLOWLIST
    ):
        fail(
            f"integration diff differs: {changed}",
            "provenance",
            "PROVENANCE_FAILURE",
        )

    return {
        "integrationBaseRef": base_ref,
        "baseCommit": base,
...
def post_merge_topology(expected_head: str):
    ...
    if parents[1] != expected_head:
        fail(
            ...
            f"expected={expected_head} actual={parents[1]}",
            "provenance",
            "PROVENANCE_FAILURE",
        )
    if not ancestor(FROZEN_BASE, parents[0]):
        fail(
            "post-merge first parent does not descend from frozen base: "
            f"{parents[0]}",
            "topology",
            "TOPOLOGY_FAILURE",
        )
    ...
    if divergence:
        fail(
            "executable tree divergence from accepted integration head: "
            f"{divergence}",
            "provenance",
...
def maintenance_topology(base_ref: str):
    ...
    if not ancestor(RELEASE_MERGE, base):
        fail(
            "maintenance base does not descend from released baseline: "
            f"{base}",
            "topology",
    ...
    if (
        ...
        fail(
            "maintenance branch is not synchronized with reviewed base: "
            f"{base}",
            "topology",
            "MAINTENANCE_TOPOLOGY_FAILURE",
        )
    ...
    if (
        len(changed) != 6
        or set(changed) != CI_ALLOWLIST
    ):
        fail(
            "maintenance diff differs from exact CI allowlist: "
            f"{changed}",
            "provenance",
    ...
    for line in filter(
        ...
        if len(parts) != 2:
            fail(
                "unsupported maintenance diff status line: "
                f"{line}",
                "provenance",
                "MAINTENANCE_SCOPE_FAILURE",
            )
    ...
    if statuses not in MAINTENANCE_STATUS_PROFILES:
        fail(
            "maintenance paths must be four modifications plus the two "
            "exact additions; statuses differ: "
            f"expected={MAINTENANCE_STATUS_PROFILES} "
            f"actual={statuses}",
            "provenance",
            "MAINTENANCE_SCOPE_FAILURE",
        )
...
def verify_input_deltas(inputs: dict[str, str]) -> dict:
    runtime_changed = set(
        filter(
            ...
            git(
                "diff",
                "--name-only",
                f"{PRODUCT_BASELINE}..{inputs['runtime']}",
            ).splitlines(),
        )
    )
    expected_runtime = {
    ...
    if (
        runtime_changed != expected_runtime
        or git(
            "merge-base",
            PRODUCT_BASELINE,
            inputs["runtime"],
        ...
        fail(
            "runtime accepted-head delta differs: "
            f"{sorted(runtime_changed)}",
            "provenance",
            "PROVENANCE_FAILURE",
        )
    ...
    if (
        ...
        fail(
            f"QA accepted-head delta differs: {sorted(qa_changed)}",
            "provenance",
            "PROVENANCE_FAILURE",
        )
    ...
    return {
        "runtime": {
            "commit": inputs["runtime"],
            "changedPaths": sorted(runtime_changed),
        },
        "qa": {
            "commit": inputs["qa"],
            "changedPaths": sorted(qa_changed),
        },
    }
...
def provenance(
    ...
    if git("status", "--porcelain"):
        fail(
            "repository dirty before checks",
            "provenance",
            "PROVENANCE_FAILURE",
        )
    ...
    schema_blob = git(
        "rev-parse",
        f"{FROZEN_BASE}:{SCHEMA}",
    )
    if (
        by_path[SCHEMA]["fingerprint"]["value"]
        != schema_blob
    ):
        fail(
            "frozen schema differs",
            "provenance",
            "PROVENANCE_FAILURE",
        )
    ...
    for owner, owned in ROLE.items():
        ...
        for path in sorted(owned):
            ...
            if blob_digest(
                ...
                fail(
                    f"declared blob cannot be reproduced: {path}",
                    "provenance",
                    "PROVENANCE_FAILURE",
                )
            ...
            if (
                owner == "runtime"
                and path in P1_PRODUCT_PATHS
            ):
                accepted = git(
                    "rev-parse",
                    f"{inputs['runtime']}:{path}",
                )
                ...
                accepted = git(
                    "rev-parse",
                    f"{inputs['authoring']}:{path}",
                )

            if (
                accepted is not None
                and declared != accepted
            ):
                fail(
                    f"{owner} accepted-head blob differs: {path}",
                    "provenance",
                    "PROVENANCE_FAILURE",
                )
            ...
            if path in P1_PRODUCT_PATHS or path == P1_TEST:
                ...
                if current != declared:
                    fail(
                        f"integrated accepted blob differs: {path}",
                        "provenance",
                        "PROVENANCE_FAILURE",
                    )

            if path == HARNESS_TEST:
                current = git(
                    "rev-parse",
                    f"HEAD:{path}",
                )
                if current != declared:
                    fail(
                        "integration harness regression-test override differs: "
                        f"{path}",
                        "provenance",
                        "PROVENANCE_FAILURE",
                    )
                files[path] = {
                    ...
                    "authorizedOverride": "PROD-WP-003 / PR #117",
                    "identical": True,
                }
            else:
                files[path] = {
        ...
        proof[owner] = {
            "commit": inputs[owner],
            "reviewId": manifest["acceptedReviews"][owner],
            "files": files,
        }

    accepted_wp = git(
        "rev-parse",
        f"{inputs['runtime']}:{WORK_PACKAGE}",
    )
    current_wp = git(
        "rev-parse",
        f"HEAD:{WORK_PACKAGE}",
    ...
    for path in INTEGRATOR - {SELF}:
        if (
            by_path[path]["fingerprint"]["value"]
    ...
    return {
        ...
        **topology,
        ...
        "workPackage": {
            "path": WORK_PACKAGE,
            "acceptedDevBlobSha1": accepted_wp,
            "materializedIntBlobSha1": current_wp,
            "identicalToAcceptedDev": accepted_wp == current_wp,
            "authorizedOverride": "complete DEV / QA / INT cycle under issue #116",
        },
        "schema": {
            "path": SCHEMA,
...
def materialize(
    ...
    # build.py remains frozen on the canonical 32-file build plan.
    # The additional P1 QA file is excluded only from this temporary
    # clean-build projection.
    ...
    projected["workingFiles"] = [
        item
        for item in projected["workingFiles"]
        if item["path"] != P1_TEST
    ]
    ...
    self_item["fingerprint"]["value"] = canonical_self_digest(
        projected
    )
...
def test_count(output: str) -> int:
    ...
    if not matches:
        fail(
            "strict QA did not report a test count:\n"
            + output,
            "product",
...
def run_exact_suite(
    ...
    for attempt in range(1, attempts + 1):
        ...
        last = result
        counts = re.findall(r"Ran\s+(\d+)\s+tests?", result["output"])
        observed = int(counts[-1]) if counts else None
        clean = not re.search(
    ...
    fail(
        f"{label} did not pass exactly {expected_tests} tests "
        f"with zero skip/failure/error in {attempts} bounded attempts:\n"
        + (last["output"] if last else "no execution result"),
        "product",
        "PRODUCT_TEST_FAILURE",
    )


def p1_browser_smoke(
    root: Path,
    artifact: Path,
) -> dict:
    script = r'''
...
fixture = json.loads(
    (root / "contracts/fixtures/v2-valid-minimal.json").read_text(
        encoding="utf-8"
    )
)
...
with artifact_server(artifact) as url:
    with sync_playwright() as playwright:
        ...
        page.wait_for_function(
            "() => Boolean(window.__LEARNIT_NEXT_TEST__)"
        )
        ...
        assert (
            session["currentActivity"]["activityRevisionId"]
            == qcm["activityRevisionId"]
        )
        ...
        assert (
            review["mode"] == "review"
            and review["currentActivity"]["activityRevisionId"]
            == qcm["activityRevisionId"]
        )
        ...
        result = api(
            "answer",
            qcm["activityRevisionId"],
            {"choiceId": qcm["correctChoiceId"]},
        )
        assert (
            result["correct"] is True
            and result["review"]["remaining"] == 0
        )
        assert api(
            "getReviewQueue",
            course_id,
    ...
    json.dumps(
        {
            "result": "PASS",
            "engine": "chromium",
            "scenario": "incorrect-review-correct-removal",
        },
        sort_keys=True,
    )
)
...
def checks(
    ...
    with tempfile.TemporaryDirectory() as raw:
            ...
            "builds": [
                {
                    ...
                    **{
                        key: value
                        for key, value in build_a.items()
                        if key != "data"
                    },
                },
                {
                    ...
                    **{
                        key: value
                        for key, value in build_b.items()
                        if key != "data"
                    },
                },
            ],
            "byteForByteIdentical": True,
            "integratedManifestBudget": 33,
            "buildProjectionBudget": 32,
        }
        ...
        need(
            p1,
            "P1 contradictory matrix",
            "product",
            "PRODUCT_TEST_FAILURE",
        )
        if (
            test_count(p1["output"]) != 4
            or re.search(
                r"skipped=|FAILED(?:\s|\()|errors?=",
                p1["output"],
                re.I,
            ...
            fail(
                "P1 contradictory test wrapper did not pass "
                "all four tests:\n"
                + p1["output"],
                "product",
                "PRODUCT_TEST_FAILURE",
        ...
        browser, browser_attempts = run_exact_suite(
            [
                sys.executable,
                "apps/learnit-next/tests/browser_vertical_slice.py",
                "-v",
        ...
        browser_total = test_count(browser["output"])
        report["browserVerticalSlice"] = {
            **browser,
            "result": "PASS",
            "engine": "chromium",
            "executed": browser_total,
            "passed": browser_total,
            "attempts": browser_attempts,
        }

        report["p1ChromiumSmoke"] = p1_browser_smoke(
            first,
            first / ARTIFACT,
        )

        qa, qa_attempts = run_exact_suite(
            [
                sys.executable,
                "-m",
                "unittest",
        ...
        if added != 19:
            fail(
                "P1 and current-topology regression count differs: "
                f"expected=19 actual={added}",
                "product",
                "PRODUCT_TEST_FAILURE",
            )
        ...
        report["qa"] = {
            **qa,
            ...
            "newP1AndTopologyRegressionTests": added,
            "skipped": 0,
            "failures": 0,
            "errors": 0,
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        default=os.environ.get(
            "LEARNIT_NEXT_ACCEPTED_INTEGRATION_HEAD",
            "",
        ),
    )
    ...
    strict = bool(
        args.strict
        or os.environ.get(
            "LEARNIT_NEXT_STRICT_INTEGRATION"
        )
        == "1"
    )
    ...
    try:
        ...
        if args.mode not in VALID_MODES:
            fail(
                "unknown or missing verification mode: "
                f"{args.mode!r}",
                "configuration",
                "CONFIGURATION_FAILURE",
            )
