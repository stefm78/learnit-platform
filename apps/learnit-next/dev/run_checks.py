...
MAINTENANCE_STATUS_PROFILES = [
    ...
    {path: ("A" if path == "apps/learnit-next/tests/build_determinism.py" else "M") for path in CI_ALLOWLIST},
]
ROLE = {
    "runtime": {
...
def validate_manifest(manifest: dict):
    ...
    if manifest.get("fileBudget") != 32 or len(items) != 32 or len(set(paths)) != 32 or set(paths) != inventory():
...
def integration_topology(base_ref: str):
    ...
    if len(changed) != 5 or set(changed) != INTEGRATOR:
        fail(f"integrator diff differs: {changed}", "provenance", "PROVENANCE_FAILURE")
...
def post_merge_topology(expected_head: str):
    ...
    return {"parents": parents, "acceptedIntegrationHead": expected_head, "firstParent": parents[0], "executableTreeDivergence": []}
...
def maintenance_topology(base_ref: str):
    ...
    if statuses not in MAINTENANCE_STATUS_PROFILES:
        fail(
            f"maintenance paths must be modifications only or the authorized baseline-absent test addition; statuses differ: expected={MAINTENANCE_STATUS_PROFILES} actual={statuses}",
    ...
    return {
        ...
        "changedPathCount": 4, "pathStatuses": dict(sorted(statuses.items())),
    }
...
def provenance(manifest: dict, mode: str, base_ref: str, expected_head: str):
    ...
    for owner, owned in ROLE.items():
        ...
        for path in sorted(owned):
            ...
            if blob_digest(git_bytes("cat-file", "blob", accepted)) != accepted:
                fail(f"accepted blob cannot be reproduced: {path}", "provenance", "PROVENANCE_FAILURE")
            if path == "apps/learnit-next/tests/build_determinism.py":
                current = git("rev-parse", f"HEAD:{path}")
                if declared != current:
                    fail(f"CI-WP-001 test fingerprint stale: {path}", "provenance", "PROVENANCE_FAILURE")
                files[path] = {"acceptedBaselineBlobSha1": accepted, "materializedBlobSha1": current, "authorizedOverride": "CI-WP-001"}
            else:
                if declared != accepted:
                    fail(f"{owner} blob differs: {path}", "provenance", "PROVENANCE_FAILURE")
    ...
    return {
        "mode": mode, "baseCommit": BASE, "sourceCommit": git("rev-parse", "HEAD"),
        **topology, "manifestBudget": 32, "roleFileCount": 26,
        "roleFiles": proof,
...
def checks(report: dict, manifest: dict) -> None:
    with tempfile.TemporaryDirectory() as raw:
        ...
        if build_a["sha256"] != manifest["artifact"]["sha256"] or build_a["bytes"] != EXPECTED_BYTES:
            fail("released artifact identity differs", "build", "BUILD_FAILURE")
        report["cleanBuilds"] = {
            "builds": [
        ...
        env = {"LEARNIT_NEXT_STRICT_INTEGRATION": "1", "LEARNIT_NEXT_ARTIFACT": str(first / ARTIFACT)}
        ...
        if added <= 0:
            fail("no topology regression tests were counted", "product", "PRODUCT_TEST_FAILURE")
...
def main() -> int:
    ...
    report = {
        "schema": "learnit.next.ci.checks.v3", "workPackage": "CI-WP-001",
