...
class FakeStorage {
  ...
  async getCourse(id) { this.calls.push(['getCourse',id]); this.maybeFail('getCourse',id); return clone(this.courses.get(id) ?? null); }
  async listProgress(courseInstallId) {
    this.calls.push(['listProgress',courseInstallId]); this.maybeFail('listProgress',courseInstallId);
  ...
  async getProgress(courseInstallId, activityRevisionId) {
    this.calls.push(['getProgress',activityRevisionId]); this.maybeFail('getProgress',activityRevisionId);
    return clone(this.progress.get(keyOf(courseInstallId,activityRevisionId)) ?? null);
  }
  async putProgress(record) {
    this.calls.push(['putProgress',record.activityRevisionId]);
  ...
  async listObjectiveProgress(courseInstallId) {
    this.calls.push(['listObjectiveProgress',courseInstallId]); this.maybeFail('listObjectiveProgress',courseInstallId);
  ...
  async putObjectiveProgressRecords(records) {
    this.calls.push(['putObjectiveProgressRecords',records.length]);
  ...
  async getMeta(key) { this.calls.push(['getMeta',key]); this.maybeFail('getMeta',key); return clone(this.meta.get(key) ?? null); }
  async setMeta(key,value) { this.calls.push(['setMeta',key]); this.meta.set(key,clone(value)); this.maybeFail('setMeta',key); }
  async deleteMeta(key) { this.calls.push(['deleteMeta',key]); this.meta.delete(key); this.maybeFail('deleteMeta',key); }
}

const objectiveProgress = {
  projectObjectiveProgress({course, activityProgress}) {
    const byActivity = new Map(activityProgress.map(record => [record.activityRevisionId,record]));
    return course.objectives.map(objective => {
      const related = course.activities.filter(activity => activity.objectiveIds.includes(objective.objectiveId));
      const training = related.filter(activity => activity.assessmentRole !== 'validation').map(activity => byActivity.get(activity.activityRevisionId)).filter(Boolean);
      const validation = related.filter(activity => activity.assessmentRole === 'validation').map(activity => byActivity.get(activity.activityRevisionId)).filter(Boolean);
      const latestTrainingCorrect = training.length ? training.at(-1).correct : null;
      const latestValidationCorrect = validation.length ? validation.at(-1).correct : null;
      const needsReview = training.some(record => record.correct === false);
      let status = 'not-started';
      if (needsReview) status = 'review-needed';
      else if (latestValidationCorrect === true) status = 'validated-recently';
      else if (training.length) status = latestTrainingCorrect ? 'ready-for-validation' : 'training';
      return {
        objectiveId:objective.objectiveId,
        trainingAttempts:training.reduce((total,record) => total + record.attempts,0),
        latestTrainingCorrect,
        needsReview,
        validationAttempts:validation.reduce((total,record) => total + record.attempts,0),
...
const learningRecommendation = {
  recommendLearningAction({objectiveProgress, reviewActivityRevisionIds}) {
    if (reviewActivityRevisionIds.length) return {kind:'corrective-review', activityRevisionIds:reviewActivityRevisionIds};
    const target = objectiveProgress.find(item => item.status !== 'validated-recently');
    return target ? {kind:'continue', objectiveId:target.objectiveId} : {kind:'complete'};
  },
};
const integrations = {objectiveProgress, learningRecommendation};
...
const seed = (storage, activity, correct=false, attempts=1) => storage.progress.set(keyOf(courseRecord.courseInstallId,activity.activityRevisionId),{
...
await test('additive objective state and exact learn index persist', async () => {
  ...
  assert.equal(result.progress.recommendation.kind,'corrective-review');
...
await test('corrective success updates objective state without durable mastery claim', async () => {
  const storage = new FakeStorage();
  seed(storage,activities[0],false);
  seed(storage,activities[1],true);
  ...
  assert.equal(result.progress.objectives[0].status,'validated-recently');
...
await test('invalid domain output fails after retaining the activity record', async () => {
  ...
  const progress = createProgressService(storage,{objectiveProgress:invalidProgress});
  const sessions = createSessionService(storage,progress);
  await assert.rejects(sessions.startCourse('course-1'));
  storage.objectiveProgress.clear();
  const good = makeRuntime(storage);
  await good.sessions.startCourse('course-1');
...
class PlatformWaveATests(unittest.TestCase):
    ...
    def test_explicit_parallel_lane_composition_seam(self) -> None:
        main = (APP / "src/main.js").read_text(encoding="utf-8")
        render = (APP / "src/ui/render.js").read_text(encoding="utf-8")
        self.assertIn("__LEARNIT_NEXT_WAVE_A__", main)
        self.assertIn("./core/objective_progress.js", main)
        self.assertIn("./core/learning_recommendation.js", main)
        self.assertIn("./ui/objective_progress.js", main)
        self.assertNotIn("from './core/objective_progress.js'", main)
    ...
    def test_scope_when_git_metadata_is_available(self) -> None:
