#!/usr/bin/env python3
"""Prove RC718 storage is unchanged after successor boot/import/session/reset."""
...
class StorageIsolationTests(unittest.TestCase):
    ...
    def assert_isolated(self,after):
    ...
    def test_boot_preserves_rc718_byte_for_byte(self):self.boot();self.assert_isolated(self.snapshot())
    def test_import_and_session_preserve_rc718_record_for_record(self):
        self.boot();self.assertFalse(negative(self.call("importPackage",self.v)));c=self.call("listCourses")[0];self.call("startCourse",c["courseInstallId"]);q=self.v["courses"][0]["activities"][0];self.call("answer",q["activityRevisionId"],q["correctChoiceId"]);self.assert_isolated(self.snapshot())
    def test_legacy_rejection_is_atomic(self):
        self.boot();before=self.snapshot();self.assertTrue(self.reject(self.l));after=self.snapshot();self.assert_isolated(after);self.assertEqual(before["next"],after["next"]);self.assertEqual([],self.call("listCourses"))
    def test_forced_successor_write_failure_is_atomic(self):
        self.page.add_init_script("""()=>{const p=IDBObjectStore.prototype.put;IDBObjectStore.prototype.put=function(...a){if(this.transaction.db.name==='learnit_next_v1')throw new DOMException('QA forced','QuotaExceededError');return p.apply(this,a)}}""");self.boot();self.assertTrue(self.reject(self.v));self.assertEqual([],self.call("listCourses"));self.assert_isolated(self.snapshot())
    def test_reset_changes_only_successor_namespaces(self):
        self.boot();self.call("importPackage",self.v);before=self.snapshot();self.call("resetNextData");after=self.snapshot();self.assert_isolated(after);self.assertEqual([],self.call("listCourses"));self.assertNotEqual(before["next"],after["next"])
