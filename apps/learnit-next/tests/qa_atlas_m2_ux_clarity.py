...
class IndependentAtlasM2UxClarityQa(unittest.TestCase):
    def test_same_action_items_have_separate_learner_objectives(self):
        result = run_node(r"""
...
assert.match(html,/<br><small class="atlas-plan-objective">Objectif : Objectif Alpha<\/small>/);
assert.match(html,/<br><small class="atlas-plan-objective">Objectif : Objectif Bêta<\/small>/);
...
""")
    ...
    def test_completed_activity_remains_visible_with_its_feedback(self):
        source = SESSION.read_text(encoding="utf-8")
        start = source.index("async function showFeedbackTransition")
        end = source.index("function assertAtlasControlVisible", start)
        transition = source[start:end]
        self.assertIn("activityWrapper", transition)
        ...
        self.assertIn("nextCheckpoint.nextItemPosition", source)
        self.assertIn("await renderCurrent();", source)
        self.assertNotIn(
            "await renderCurrent(\n              feedbackHtml(",
            source,
        )
