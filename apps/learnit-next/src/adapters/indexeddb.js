...
function openDatabase(indexedDbApi) {
  return new Promise((resolve, reject) => {
    ...
    request.addEventListener('upgradeneeded', () => {
      ...
      if (!database.objectStoreNames.contains(NEXT_OBJECTIVE_PROGRESS_STORE)) {
        const objectiveProgress = database.createObjectStore(NEXT_OBJECTIVE_PROGRESS_STORE, {
          keyPath: ['courseInstallId', 'objectiveId'],
        });
        objectiveProgress.createIndex('courseInstallId', 'courseInstallId', { unique: false });
      }
    });
...
export function createIndexedDbStorage({
  ...
  return {
    ...
    async listObjectiveProgress(courseInstallId) {
      const db = await database();
      const transaction = db.transaction(NEXT_OBJECTIVE_PROGRESS_STORE, 'readonly');
      const store = transaction.objectStore(NEXT_OBJECTIVE_PROGRESS_STORE);
      const records = await requestResult(store.index('courseInstallId').getAll(courseInstallId));
      await transactionDone(transaction);
      return records.sort((left, right) => left.objectiveId.localeCompare(right.objectiveId));
    },

    async putObjectiveProgressRecords(records) {
      if (!Array.isArray(records)) throw new TypeError('Objective progress records must be an array');
      if (records.length === 0) return;
      const db = await database();
      const transaction = db.transaction(NEXT_OBJECTIVE_PROGRESS_STORE, 'readwrite');
      const completion = transactionDone(transaction);
      const store = transaction.objectStore(NEXT_OBJECTIVE_PROGRESS_STORE);
      try {
