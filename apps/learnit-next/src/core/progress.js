...
import { isNonScoredActivity } from './activity_semantics.js';
...
export function deriveReviewQueue(course, records) {
  ...
  return course.activities.filter(activity => byActivity.get(activity.activityRevisionId)?.correct === false);
}
...
export function summarizeProgress(course, records) {
  ...
  return {
    ...
    records: course.activities.map(activity => byActivity.get(activity.activityRevisionId)).filter(Boolean),
  };
}
...
export function buildCompletionRecord({ courseInstallId, activity, answer, previous, now = new Date() }) {
  if (!isNonScoredActivity(activity)) throw new TypeError('recordCompletion() accepts only lesson or flashcard');
  return {
    courseInstallId,
    activityLineageId: activity.activityLineageId,
    activityRevisionId: activity.activityRevisionId,
...
function requireDomainFunction(moduleValue, exportName, moduleLabel) {
  ...
  if (typeof candidate !== 'function') throw new TypeError(`Learning Loop V2 ${moduleLabel}.${exportName}() is required`);
  return candidate.bind(moduleValue);
}

function requireAuthoredCourse(course) {
  if (!course || typeof course !== 'object' || Array.isArray(course)) throw new TypeError('Learning Loop V2 authored course must be an object');
  if (!Array.isArray(course.objectives) || !Array.isArray(course.activities)) throw new TypeError('Learning Loop V2 authored course requires objectives and activities arrays');
  const objectiveIds = [];
  const objectiveIdSet = new Set();
  for (const objective of course.objectives) {
    const objectiveId = objective?.objectiveId;
    if (typeof objectiveId !== 'string' || objectiveId.trim() === '') throw new TypeError('Learning Loop V2 authored objectiveId must be a non-empty string');
    if (objectiveIdSet.has(objectiveId)) throw new TypeError(`Learning Loop V2 duplicate authored objectiveId: ${objectiveId}`);
    objectiveIdSet.add(objectiveId); objectiveIds.push(objectiveId);
  }
  const activities = [];
  ...
  course.activities.forEach((activity, authorIndex) => {
    ...
    if (typeof activityRevisionId !== 'string' || activityRevisionId.trim() === '') throw new TypeError('Learning Loop V2 activityRevisionId must be a non-empty string');
    if (activityByRevisionId.has(activityRevisionId)) throw new TypeError(`Learning Loop V2 duplicate activityRevisionId: ${activityRevisionId}`);
    if (!Array.isArray(activity.objectiveIds) || activity.objectiveIds.length === 0) throw new TypeError(`Learning Loop V2 activity ${activityRevisionId} requires objectiveIds`);
    const seen = new Set();
    for (const objectiveId of activity.objectiveIds) {
      if (typeof objectiveId !== 'string' || !objectiveIdSet.has(objectiveId) || seen.has(objectiveId)) {
        throw new TypeError(`Learning Loop V2 activity ${activityRevisionId} contains an unknown or duplicate objectiveId`);
      }
...
function requireActivityProgress(activityProgress, activityByRevisionId) {
  if (!Array.isArray(activityProgress)) throw new TypeError('Learning Loop V2 activityProgress must be an array');
  const entries = [];
  const seen = new Set();
  for (const record of activityProgress) {
    if (!record || typeof record !== 'object' || Array.isArray(record)) throw new TypeError('Learning Loop V2 activity progress record must be an object');
    const activityRevisionId = record.activityRevisionId;
    const authored = activityByRevisionId.get(activityRevisionId);
    if (!authored || seen.has(activityRevisionId)) throw new TypeError(`Learning Loop V2 progress contains an unknown or duplicate activityRevisionId: ${String(activityRevisionId)}`);
    if (record.completed !== true) throw new TypeError(`Learning Loop V2 progress ${activityRevisionId} must be completed`);
    if (!Number.isInteger(record.attempts) || record.attempts < 1) throw new TypeError(`Learning Loop V2 progress ${activityRevisionId} has invalid attempts`);
    const nonScored = isNonScoredActivity(authored.activity);
    if (nonScored) {
      if (record.scored !== false || Object.hasOwn(record, 'correct')) {
        throw new TypeError(`Learning Loop V2 non-scored progress ${activityRevisionId} must not carry correctness`);
      }
    } else if (typeof record.correct !== 'boolean') {
      throw new TypeError(`Learning Loop V2 progress ${activityRevisionId} has invalid correct`);
    }
    if (typeof record.updatedAt !== 'string' || record.updatedAt.trim() === '') throw new TypeError(`Learning Loop V2 progress ${activityRevisionId} has invalid updatedAt`);
    seen.add(activityRevisionId); entries.push({ ...authored, record, nonScored });
  }
...
export function createLearningLoopV2DomainAdapters(objectiveProgressModule, learningRecommendationModule) {
  const reduceObjectiveEvents = requireDomainFunction(objectiveProgressModule, 'reduceObjectiveEvents', 'objectiveProgress');
  const normalizeObjectiveProgress = requireDomainFunction(objectiveProgressModule, 'normalizeObjectiveProgress', 'objectiveProgress');
  const recommendNextObjective = requireDomainFunction(learningRecommendationModule, 'recommendNextObjective', 'learningRecommendation');
  const objectiveProgress = Object.freeze({
    projectObjectiveProgress({ course, activityProgress }) {
      ...
      const progressEntries = requireActivityProgress(activityProgress, authored.activityByRevisionId);
  ...
  const learningRecommendation = Object.freeze({
    recommendLearningAction({ course, objectiveProgress: records }) {
      const authored = requireAuthoredCourse(course);
      if (!Array.isArray(records)) throw new TypeError('Learning Loop V2 objectiveProgress must be an array');
...
function normalizeObjectiveProjection(courseInstallId, course, projected, existing, now) {
  if (!Array.isArray(projected)) throw new TypeError('projectObjectiveProgress() must return an array');
  ...
  if (projected.length !== authoredIds.length) throw new TypeError('Objective projection must contain every authored objective exactly once');
  ...
  for (const state of projected) {
    ...
    if (typeof objectiveId !== 'string' || !authoredIds.includes(objectiveId) || projectedById.has(objectiveId)) throw new TypeError('Objective projection contains an unknown or duplicate objectiveId');
    const normalized = {
      objectiveId,
      ...
      needsReview: typeof state.needsReview === 'boolean' ? state.needsReview : (() => { throw new TypeError(`Objective ${objectiveId} has invalid needsReview`); })(),
...
function normalizeRecommendation(value) {
  ...
  if (typeof value !== 'object' || Array.isArray(value)) throw new TypeError('recommendLearningAction() must return a data object or null');
...
export function assertLearningLoopV2Integrations(integrations = {}) {
  ...
  if (objectiveProgress == null && learningRecommendation == null) return Object.freeze({ objectiveProgress: null, learningRecommendation: null });
  if (typeof objectiveProgress?.projectObjectiveProgress !== 'function') throw new TypeError('Learning Loop V2 objectiveProgress.projectObjectiveProgress() is required');
  if (learningRecommendation != null && typeof learningRecommendation.recommendLearningAction !== 'function') throw new TypeError('Learning Loop V2 learningRecommendation.recommendLearningAction() is required');
  return Object.freeze({ objectiveProgress, learningRecommendation });
}
...
export function createProgressService(storage, integrations = {}) {
  ...
  async function projectCourseProgress(courseInstallId, course, records = null) {
    ...
    const projected = await resolved.objectiveProgress.projectObjectiveProgress({
      course: structuredClone(course), activityProgress: structuredClone(activityProgress),
    });
    const objectiveRecords = normalizeObjectiveProjection(courseInstallId, course, projected, existing, new Date());
    if (!sameObjectiveRecords(existing, objectiveRecords)) await storage.putObjectiveProgressRecords(objectiveRecords);
    const objectiveProgress = objectiveRecords.map(record => {
      ...
      void schemaVersion; void storedCourseInstallId; void updatedAt; return state;
    });
    const reviewActivityRevisionIds = deriveReviewQueue(course, activityProgress).map(activity => activity.activityRevisionId);
    const recommendation = resolved.learningRecommendation
      ...
      : null;
    return { ...legacy, objectives: objectiveProgress, recommendation };
  }

  async function storeAndProject(input, record) {
    await storage.putProgress(record);
    if (!learningLoopV2Enabled) return record;
    if (!input.course) throw new TypeError('Learning Loop V2 progress recording requires the authored course');
    ...
    return { ...record, objectiveProgress: projection.objectives, recommendation: projection.recommendation };
  }

  return Object.freeze({
    learningLoopV2Enabled,
    async getProgress(courseInstallId) { return storage.listProgress(courseInstallId); },
    async getCourseProgress(courseInstallId, course, records = null) { return projectCourseProgress(courseInstallId, course, records); },
    async getObjectiveProgress(courseInstallId, course) {
      const projection = await projectCourseProgress(courseInstallId, course);
      return { courseInstallId, objectives: projection.objectives ?? [], recommendation: projection.recommendation ?? null };
    },
    async recordAttempt(input) {
      if (isNonScoredActivity(input.activity)) throw new TypeError('lesson/flashcard must use recordCompletion()');
      const previous = await storage.getProgress(input.courseInstallId, input.activity.activityRevisionId);
      return storeAndProject(input, buildProgressRecord({ ...input, previous }));
    },
    async recordCompletion(input) {
      if (!isNonScoredActivity(input.activity)) throw new TypeError('recordCompletion() accepts only lesson or flashcard');
      const previous = await storage.getProgress(input.courseInstallId, input.activity.activityRevisionId);
      return storeAndProject(input, buildCompletionRecord({ ...input, previous }));
    },
