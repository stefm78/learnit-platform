...
export const CONTRACT_V3_VERSION = 'learnit.kit.v3';
const DIGEST_PATTERN = /^sha256:[0-9a-f]{64}$/;
const TYPED_CLAIM_ID_PATTERN = /^atlas-claim-sha256:[0-9a-f]{64}$/;
...
const INDEPENDENCE_BASES = new Set(['new-instance', 'new-context', 'alternate-representation']);
...
const V3_COURSE_KEYS = new Set([...COURSE_KEYS, 'atlasValidationIndependenceClaims']);
const V3_QCM_KEYS = new Set([...QCM_KEYS, 'estimatedMinutes']);
const V3_FILL_KEYS = new Set([...FILL_KEYS, 'estimatedMinutes']);
const V3_CONSTRUCTED_KEYS = new Set([...ACTIVITY_COMMON_KEYS, 'estimatedMinutes', 'acceptedResponses']);
const INDEPENDENCE_CLAIM_KEYS = new Set([
  'claimVersion',
  'claimId',
  'objectiveId',
  'sourceActivityLineageId',
  'targetActivityLineageId',
  'basisCode',
  'sourceStimulusDigest',
  'targetStimulusDigest',
]);
...
function exactKeysV3(value, allowed, required, path, errors) {
  for (const key of required) {
    if (!Object.hasOwn(value, key)) issue(errors, 'required', `${path}.${key}`, 'Required property is missing');
  }
  for (const key of Object.keys(value)) {
    if (!allowed.has(key)) issue(errors, 'additional_property', `${path}.${key}`, 'Property is not allowed by learnit.kit.v3');
  }
}
...
function validateIndependenceClaimShapeV3(claim, path, errors) {
  if (!requireObject(claim, path, errors)) return;
  exactKeysV3(claim, INDEPENDENCE_CLAIM_KEYS, INDEPENDENCE_CLAIM_KEYS, path, errors);
  stringValue(claim.claimVersion, `${path}.claimVersion`, errors, { constant: 'atlas.independence.v1' });
  stringValue(claim.claimId, `${path}.claimId`, errors, { pattern: TYPED_CLAIM_ID_PATTERN });
  uuidValue(claim.objectiveId, `${path}.objectiveId`, errors);
  uuidValue(claim.sourceActivityLineageId, `${path}.sourceActivityLineageId`, errors);
  uuidValue(claim.targetActivityLineageId, `${path}.targetActivityLineageId`, errors);
  enumValue(claim.basisCode, INDEPENDENCE_BASES, `${path}.basisCode`, errors);
  digestValue(claim.sourceStimulusDigest, `${path}.sourceStimulusDigest`, errors);
  digestValue(claim.targetStimulusDigest, `${path}.targetStimulusDigest`, errors);
}

function validateQcmShapeV3(activity, path, errors) {
  const required = new Set(QCM_KEYS);
  exactKeysV3(activity, V3_QCM_KEYS, required, path, errors);
  const projected = { ...activity };
  delete projected.estimatedMinutes;
  validateQcmShape(projected, path, errors);
  if (Object.hasOwn(activity, 'estimatedMinutes')) integerValue(activity.estimatedMinutes, `${path}.estimatedMinutes`, errors, { min: 1, max: 30 });
}

function validateFillShapeV3(activity, path, errors) {
  const required = new Set(FILL_KEYS);
  exactKeysV3(activity, V3_FILL_KEYS, required, path, errors);
  const projected = { ...activity };
  delete projected.estimatedMinutes;
  validateFillShape(projected, path, errors);
  if (Object.hasOwn(activity, 'estimatedMinutes')) integerValue(activity.estimatedMinutes, `${path}.estimatedMinutes`, errors, { min: 1, max: 30 });
}

function validateConstructedShapeV3(activity, path, errors) {
  const required = new Set([...ACTIVITY_COMMON_KEYS, 'acceptedResponses']);
  exactKeysV3(activity, V3_CONSTRUCTED_KEYS, required, path, errors);
  validateActivityCommonShape(activity, path, errors);
  stringValue(activity.type, `${path}.type`, errors, { constant: 'constructed' });
  if (Object.hasOwn(activity, 'estimatedMinutes')) integerValue(activity.estimatedMinutes, `${path}.estimatedMinutes`, errors, { min: 1, max: 30 });
  if (arrayValue(activity.acceptedResponses, `${path}.acceptedResponses`, errors, { min: 1, max: 20 })) {
    const seen = new Set();
    activity.acceptedResponses.forEach((response, index) => {
      const responsePath = `${path}.acceptedResponses[${index}]`;
      stringValue(response, responsePath, errors, { min: 1, max: 4000 });
      if (seen.has(response)) issue(errors, 'unique_items', responsePath, 'Accepted response is duplicated');
      seen.add(response);
    });
  }
}

function validateActivityShapeV3(activity, path, errors) {
  if (!requireObject(activity, path, errors)) return;
  if (activity.type === 'qcm') validateQcmShapeV3(activity, path, errors);
  else if (activity.type === 'fill') validateFillShapeV3(activity, path, errors);
  else if (activity.type === 'constructed') validateConstructedShapeV3(activity, path, errors);
  else issue(errors, 'activity_type', `${path}.type`, 'Only qcm, fill and constructed activities are supported');
}

function validateCourseShapeV3(course, path, errors) {
  if (!requireObject(course, path, errors)) return;
  const required = new Set([...COURSE_KEYS].filter((key) => key !== 'subtitle'));
  exactKeysV3(course, V3_COURSE_KEYS, required, path, errors);
  uuidValue(course.courseLineageId, `${path}.courseLineageId`, errors);
  uuidValue(course.courseRevisionId, `${path}.courseRevisionId`, errors);
  digestValue(course.courseRevisionDigest, `${path}.courseRevisionDigest`, errors);
  stringValue(course.title, `${path}.title`, errors, { min: 1, max: 180 });
  ...
  if (arrayValue(course.activities, `${path}.activities`, errors, { min: 1, max: 200 })) {
    course.activities.forEach((activity, index) => validateActivityShapeV3(activity, `${path}.activities[${index}]`, errors));
  }
  if (Object.hasOwn(course, 'atlasValidationIndependenceClaims')) {
    if (arrayValue(course.atlasValidationIndependenceClaims, `${path}.atlasValidationIndependenceClaims`, errors, { max: 500 })) {
      course.atlasValidationIndependenceClaims.forEach((claim, index) => validateIndependenceClaimShapeV3(claim, `${path}.atlasValidationIndependenceClaims[${index}]`, errors));
    }
  }
}

function validatePackageShapeV3(payload, errors) {
  if (!requireObject(payload, '$', errors)) return;
  const required = new Set([...PACKAGE_KEYS].filter((key) => key !== 'description'));
  exactKeysV3(payload, PACKAGE_KEYS, required, '$', errors);
  stringValue(payload.contract, '$.contract', errors, { constant: CONTRACT_V3_VERSION });
  uuidValue(payload.packageLineageId, '$.packageLineageId', errors);
  uuidValue(payload.packageRevisionId, '$.packageRevisionId', errors);
  digestValue(payload.packageRevisionDigest, '$.packageRevisionDigest', errors);
  stringValue(payload.title, '$.title', errors, { min: 1, max: 180 });
  ...
  stringValue(payload.language, '$.language', errors, { pattern: LANGUAGE_PATTERN });
  if (arrayValue(payload.courses, '$.courses', errors, { min: 1, max: 20 })) {
    payload.courses.forEach((course, index) => validateCourseShapeV3(course, `$.courses[${index}]`, errors));
  }
}
...
export async function validatePackageV3Object(payload, { existingRevisionDigests } = {}) {
  const errors = [];
  validatePackageShapeV3(payload, errors);
  if (errors.length > 0) return { ok: false, contractVersion: CONTRACT_V3_VERSION, errors };
  ...
  return {
    ok: errors.length === 0,
    contractVersion: CONTRACT_V3_VERSION,
    errors,
    revisionDigests: new Map([...revisions].map(([id, entry]) => [id, entry.digest])),
  };
}

export async function assertValidPackageV3(payload, options) {
  const result = await validatePackageV3Object(payload, options);
  if (!result.ok) throw new ContractValidationError(result.errors);
  return result;
}
