    ...
    const RouteViewComposer = (()=>{
      ...
      function collectionRenameControls(runtime,g,index){if(g.kind!=='imported')return {trigger:'',editor:''};const active=runtime.libraryRenameCollectionKey===g.key;const trigger=active?'':`<button type="button" class="book-secondary" data-action="library-rename-collection" data-collection="${attr(g.key)}" aria-label="Renommer le plan ${attr(g.label)}">Renommer</button>`;if(!active)return {trigger,editor:''};const message=runtime.libraryRenameCollectionMessage?`<p class="tiny ${runtime.libraryRenameCollectionMessage.startsWith('Erreur')?'warn':''}">${esc(runtime.libraryRenameCollectionMessage)}</p>`:'';const editor=`<div class="course-rename-editor"><label for="collectionRenameInput">Nom du plan</label><input id="collectionRenameInput" value="${attr(runtime.libraryRenameCollectionDraft||g.label)}" maxlength="120" autocomplete="off"><div class="actions"><button type="button" class="primary" data-action="library-rename-collection-save" data-collection="${attr(g.key)}">Enregistrer</button><button type="button" data-action="library-rename-collection-cancel">Annuler</button></div>${message}</div>`;return {trigger,editor};}
